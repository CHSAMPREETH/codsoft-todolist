...
class ToDoListApp:
    def __init__(self, root):
        ...
        self.taskInput = tkn.Entry(self.entryFrame, width=40, background="white", borderwidth=0.1)
        self.taskInput.grid(row=0, column=0, padx=10)
        self.addTaskBtn = tkn.Button(self.entryFrame, text=" Add Task ", command=self.add_task, bg="green", foreground="white", borderwidth=0)
        ...
        self.priority_listbox = tkn.Listbox(self.listFrm, width=10, height=15, yscrollcommand=self.scrollBar.set, borderwidth=0.1)
        self.priority_listbox.grid(row=1, column=0)
        self.tasksbox = tkn.Listbox(self.listFrm, width=40, height=15, yscrollcommand=self.scrollBar.set, borderwidth=0.1)
        ...
        self.upBtn = tkn.Button(self.priorityPanel, text="↑", command=self.moveUp, bg="orange", borderwidth=0.1)
        self.upBtn.grid(row=0, column=0)

        self.downBtn = tkn.Button(self.priorityPanel, text="↓", command=self.moveDown, bg="orange", borderwidth=0.1)
        self.downBtn.grid(row=0, column=1)

        self.delete_button = tkn.Button(self.priorityPanel, text="  Delete Task  ", command=self.delete_task, bg="red", foreground="white", borderwidth=0.1)
